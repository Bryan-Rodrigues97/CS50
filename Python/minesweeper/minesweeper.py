...
class MinesweeperAI():
    ...
    def mark_mine(self, cell):
        ...
        if cell not in self.mines:
            self.mines.add(cell)
            # Atualiza todas as sentenças conhecidas
    ...
    def add_neighbors(self, cell, count):
        """
        Adiciona as células vizinhas não conhecidas a uma nova sentença
        """
        neighbors = set()
        for i in range(cell[0] - 1, cell[0] + 2):
            for j in range(cell[1] - 1, cell[1] + 2):
        ...
        if neighbors:
            ...
            # Evita adicionar sentenças redundantes
            if new_sentence not in self.knowledge:
                self.knowledge.append(new_sentence)

    def mark_safes_mines(self):
        """
        Atualiza as células conhecidas como seguras ou minas com base nas sentenças
        """
        changed = True
        while changed:
            changed = False
            ...
            if mines_to_mark:
                for mine in mines_to_mark.copy():
                    if mine not in self.mines:
                        self.mark_mine(mine)
                        changed = True

            if safes_to_mark:
                for safe in safes_to_mark.copy():
    ...
    def update_knowledge(self):
        """
        Remove sentenças vazias e redundantes da base de conhecimento
        """
        new_knowledge = []
        for sentence in self.knowledge:
            if sentence.cells:
    ...
    def infer_new_sentences(self):
        """
        Infer novas sentenças a partir das sentenças existentes
        """
        new_sentences = []
    ...
    def add_knowledge(self, cell, count):
        ...
        self.mark_safes_mines()
        self.update_knowledge()

        # 5. add any new sentences to the AI's knowledge base if they can be inferred from existing knowledge
        self.infer_new_sentences()

        # Após inferir novas sentenças, marque novamente safes e mines caso novas informações tenham sido descobertas
        self.mark_safes_mines()
        self.update_knowledge()
