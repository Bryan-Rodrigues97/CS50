...
bool print_winner(void)
{
    ...
    if(maxVotes > (voter_count/2))
    {
        for(int i = 0; i < candidate_count; i++)
        {
            if(candidates[i].eliminated == false && candidates[i].votes == maxVotes)
            {
                printf("%s\n", candidates[i].name);
            }
        }
...
bool is_tie(int min)
{
    ...
    for(int i = 0; i < candidate_count; i++)
    {
        if(candidates[i].eliminated == false)
        {
            if(first)
            {
                prevCandidateVotes = candidates[i].votes;
                first = false;
            }
            else
            {
                if(candidates[i].votes != prevCandidateVotes)
                {
                    return false;
                }
            }
